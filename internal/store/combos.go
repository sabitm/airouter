...
SELECT t.combo_id, t.id, t.provider_id, t.upstream_model, t.position, t.enabled,
       p.id, p.name, p.base_url, p.api_key, p.protocol, p.auth_scheme, p.auth_method, p.oauth_creds, p.reasoning_dialect, p.tags, p.created_at, p.updated_at, p.archived
...
ORDER BY t.combo_id, t.position, t.id`
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&p.ID, &p.Name, &p.BaseURL, &enc, &p.Protocol, &p.AuthScheme, &p.AuthMethod, &oauthEnc, &dialect, &tagsJSON, &p.CreatedAt, &p.UpdatedAt, &p.Archived,
		); err != nil {
			return err
		}
