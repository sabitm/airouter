...
func TestCapsForOpencodeCatalog(t *testing.T) {
	cases := []struct {
		model      string
		reasoning  bool
		format     Format
		...
		{"minimax-m3", false, FormatNone, true},
		{"mimo-v2.5-pro", false, FormatNone, true},
		{"glm-5", false, FormatNone, true},
		{"glm-5.2", true, FormatOpenAI, false},
		{"kimi-k3", true, FormatOpenAI, false},
		{"muse-spark-1.2", true, FormatOpenAIResponses, false},
		{"gpt-5.4", true, FormatOpenAIResponses, true},
		{"unknown-model", false, FormatNone, true},
	}
	for _, tc := range cases {
...
func TestApplyWireOpencodeCatalog(t *testing.T) {
	...
	out, err := ApplyWire("oai-chat", body, "glm-5.2", &Config{Mode: ModeLevel, Level: "low"}, domain.ProtocolOpencode, domain.ReasoningOpencode)
	if err != nil {
		t.Fatal(err)
	}
	var m map[string]any
	...
	if m["reasoning_effort"] != "high" {
		t.Fatalf("glm-5.2 low = %v, want high", m["reasoning_effort"])
	}
	if _, ok := m["thinking"]; ok {
	...
	if oc, ok := m["output_config"].(map[string]any); ok && oc["format"] == nil {
		t.Fatalf("output_config sibling lost: %s", out)
	}

	none, err := ApplyWire("oai-chat", body, "minimax-m3", &Config{Mode: ModeLevel, Level: "high"}, domain.ProtocolOpencode, domain.ReasoningOpencode)
	if err != nil {
		t.Fatal(err)
	}
	m = map[string]any{}
	if err := json.Unmarshal(none, &m); err != nil {
		t.Fatal(err)
	}
	if effort, ok := m["reasoning_effort"]; ok {
		t.Fatalf("toggle-only model wrote reasoning_effort: %v body=%s", effort, none)
	}
	if oc, ok := m["output_config"].(map[string]any); ok {
		if _, has := oc["effort"]; has {
			t.Fatalf("toggle-only model wrote output effort: %s", none)
		}
	}
	if _, ok := m["thinking"]; ok {
		t.Fatalf("toggle-only model wrote thinking: %s", none)
	}
}
